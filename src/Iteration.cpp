...
void Iteration::pzeta_pt(vvmArray &model) {
	...
	for (int i = 1; i <= model.nx-2; i++) {
		for (int k = 2; k <= model.nz-2; k++) {
			...
			#else
				#if defined(FLUXFORM)
					model.zetap[i][k] = model.zetam[i][k] + d2t * (g_tbrho_pth_px - upzeta_px - zetapu_px - wpzeta_pz - zetapw_pz - wzeta_rhoprho_pz);
				#else
					model.zetap[i][k] = model.zetam[i][k] + d2t * (g_tbrho_pth_px - upzeta_px - wpzeta_pz);
				#endif
...
void Iteration::pth_pt(vvmArray &model) {
	#if defined(FLUXFORM)
    	double upth_px = 0., thpu_px = 0., wpth_pz = 0., thpw_pz = 0., wth_rhoprho_pz = 0.;
	#else
	...
	for (int i = 1; i <= model.nx-2; i++) {
		for (int k = 1; k <= model.nz-2; k++) {
			...
			wpth_pz = 0.5*(model.w[i][k+1]+model.w[i][k]) * (model.th[i][k+1] - model.th[i][k-1]) * model.r2dz;
				...
				#else
					#if defined(FLUXFORM)
						model.thp[i][k] = model.thm[i][k] + d2t * (-upth_px - thpu_px - wpth_pz - thpw_pz - wth_rhoprho_pz + forcing);
						// model.thp[i][k] = model.thm[i][k] + d2t * (-upth_px - wpth_pz + forcing);
					#else
						model.thp[i][k] = model.thm[i][k] + d2t * (-upth_px - wpth_pz + forcing);
					#endif
	...
	#if defined(HEATFLUX)
		for (int i = 1; i <= model.nx-2; i++) {
			if (i <= model.nx / 2 - 1) heatflux(model, i, 1, 1);
		}
	#endif
...
void Iteration::pqr_pt(vvmArray &model) {
	...
	for (int k = 1; k <= model.nz-2; k++) {
		for (int i = 1; i <= model.nx-2; i++) {
			double VT = 36.34 * pow(model.rhou[k]*model.qr[i][k], 0.1346) * pow(model.rhou[k]/model.rhow[1], -0.5);
			#if defined(FLUXFORM)
